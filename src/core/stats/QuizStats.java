...
public class QuizStats implements Serializable {
	...
	/**
	 * Percentage of total cards looked at during session that user selected
	 * 'final'.
	 */
	...
	public QuizStats(int numAgain, int numFinal, int percentCorrect, int timeTaken) {
		this.numAgain = this.numFinal = this.percentCorrect = calcPercentCorrect(this.numAgain, this.numFinal);
	 ...
	 * 
	 * @return int representing percentage of total cards looked at during session
	 *         that user selected 'final'.
	 */
