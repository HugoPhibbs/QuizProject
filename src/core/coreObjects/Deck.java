...
public class Deck implements Serializable {
	...
	/**
	 * Determines if two Deck objects are equal to each other. Checks if their name
	 * and size are equal.
	 * 
	 */
	@Override
	 ...
	 * <p>
	 * Creates a temporary FlashCard object and checks if this is in the flashCards
	 * for this deck
	 * 
	 * @param frontText String for the front text of a FlashCard to be check if it
	 *                  is contained in this deck
	 * @param backText  String for the back text of a FlashCard to be check if it is
	 ...
	 * <p>
	 * Will not add a FlashCard to a deck if a FlashCard with same front and back
	 ...
	 * @return boolean if the flashCard was removed from cards, i.e. it was found or
	 *         not.
	 */
	 ...
	 * <p>
	 * Makes sure that another FlashCard doesn't already have the same front and
	 * back text that is wished to be edited to for flashCard. Throws an exception
	 * if so.
	 * <p>
	 * Additionally, if the new front and back text's are the same as the texts of
	 * flashCard, ie the front and back text's aren't actually being changed, it
	 * returns true, and flashCard isn't changed
	 * 
	 ...
	 * @throws IllegalArgumentException if: <br>
	 *                                  - Variable flashCard isn't contained in this
	 *                                  deck <br>
	 *                                  - New front and back text already matches a
	 *                                  FlashCard in this deck
	 */
	public boolean editFlashCard(FlashCard flashCard, String newFrontText, String newBackText) {
		if (!contains(flashCard)) {
			throw new IllegalArgumentException(
					"FlashCard object isn't contained in this deck!, Please check for bugs!");
		} else if (flashCard.hasText(newFrontText, newBackText)) {
			return true;
		} else if (contains(newFrontText, newBackText)) {
			// After statement above, this checks for another flashCard with newFrontText
			// and newBackText
	...
	/**
	 * Creates and ArrayList containing new FlashCards from this Deck, adds new
	 * cards to the resultant ArrayList as long as the number of added new cards is
	 * less than maxNewCards
	 * 
	 * @param maxNewCards int for the max number of new FlashCards to be added to
	 ...
	 * <p>
	 * Again borrows functionality from ArrayList, as this will often be called and
	 * is cleaner than Deck.getflashCards.size()
	 * <p>
	 ...
	 * @throws IllegalArgumentException if name isnt' valid, see
